    ...
    for item in dealers_price.index:
        if item in glob_price.index:
            if glob_price.loc[item]['Розпродаж'] != np.nan:
                dealers_price.loc[item, ['Розпродаж']] = \
                    glob_price.loc[item]['Розпродаж']
    return dealers_price
...
Як працює: аналізує файли постачальника, які Ви їй надаєте.
   1. Файл залишків на складі постачальника.
   2. Актуальний прайс постачальника.
Потім зберігає копію прайса дилера з деякими змінами:
...
закінчення.
   3. Якщо товар сьогодні в розпродажу - додается стовпчик "Розпродаж", знаком "+" в ньому
маркуються відповідні товари
